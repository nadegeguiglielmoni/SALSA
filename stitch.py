        ...
        if curr_utg not in unitig2contig:
            ...
            if to_check_f:
                while contig_path[utg_index_f] == path_normal[curr_ind_f]:
                    ...
                    curr_ind_f += 1
                    fcount += 1
                    if utg_index_f >= len(contig_path) or curr_ind_f >= len(
                        path_normal
                    ):
                        break
                ...
                if fcount > 1:
                    new_path.append(
                        [unitig2contig[curr_utg], start_coord_f, end_coord_f, "FOW"]
                    )
                    i = curr_ind_f
                    continue
            ...
            if rcount > 1:
                new_path.append(
                    [unitig2contig[curr_utg], start_coord_r, end_coord_r, "REV"]
                )
                i = curr_ind_r
            else:
                new_path.append(path_with_ori[i])
