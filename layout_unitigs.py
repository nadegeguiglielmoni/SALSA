...
parser.add_argument(
    "-x", "--graph", help="GFA file for assembly graph", required=False, default="abc"
)
parser.add_argument(
    "-l", "--links", help="Links sorted by relative score", required=True
)
parser.add_argument(
    "-c",
    "--cutoff",
    help="Minimum length contig to consider for scaffolding",
    required=False,
    default=1000,
)
parser.add_argument(
...
parser.add_argument(
    "-t",
    "--tenx",
    help="Links obtained from 10x file sorted by last column",
    required=False,
    default="abc",
)
parser.add_argument(
    "-d", "--directory", help="Output Directory", required=False, default="out"
)
...
def load_GFA(path):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if line[0] == "S":
                OVl_G.add_node(
                    line[1] + "$REV", name=line[1], length=line[3].split(":")[2]
                )
                OVl_G.add_node(
...
def load_tenx_graph(cutoff):
    ...
    with open(args.tenx, "r") as f:
        for line in f:
            ...
            v2 = attrs[1]
            if (
                float(attrs[4]) >= cutoff
                and v1 not in tenx_graph.nodes()
                and v2 not in tenx_graph.nodes()
            ):
...
if int(args.iteration) > 1:
    try:
        previous_scaffolds = pickle.load(
            open(
                args.directory + "/scaffolds_iteration_" + str(iteration - 1) + ".p",
                "r",
            )
        )
        ...
        if args.tenx != "abc":
            for u, v in tenx_graph.edges():
                ...
                if contig_1 in contig2scaffold and contig_2 in contig2scaffold:
                    ...
                    test_edge(
                        previous_scaffolds[scaffold_1],
                        previous_scaffolds[scaffold_2],
                        tenx_graph,
                        "tenx",
                    )
...
def test_edge(scaffold_first, scaffold_second, G_test, type):
    ...
    if G_test.has_edge(
        reverse_complement(first_first), reverse_complement(last_second)
    ):
        v1 = c1 + ":E"
        v2 = c2 + ":B"
        if v1 not in G.nodes() and v2 not in G.nodes():
...
def generate_scaffold_graph():
    ...
    tiling_edges = 0
    with open(
        args.directory + "/contig_links_scaled_sorted_iteration_" + str(iteration), "r"
    ) as f:
        for line in f:
            # i += 1
            # print(i)
            ...
            # filter out links
            if iteration == 1:
                if contig_length[c1] <= int(args.cutoff) or contig_length[c2] <= int(
                    args.cutoff
                ):
                    continue
    ...
    print(
        "Hybrid scaffold graph loaded, nodes = "
        + str(len(G.nodes()))
        + " edges = "
        + str(len(G.edges())),
        file=sys.stderr,
...
def insert(assignment, seed_scaffolds):
    ...
    for contig in assignment:
        ...
        for i in xrange(1, len(path) - 1, 2):
            score_fow = -1
            score_rev = -1
            if all_G.has_edge(five_prime, path[i]) and all_G.has_edge(
                three_prime, path[i + 1]
            ):
                score_fow = (
                    all_G[five_prime][path[i]]["score"]
                    + all_G[three_prime][path[i + 1]]["score"]
                )
            if all_G.has_edge(three_prime, path[i]) and all_G.has_edge(
                five_prime, path[i + 1]
            ):
                score_rev = (
                    all_G[three_prime][path[i]]["score"]
                    + all_G[five_prime][path[i + 1]]["score"]
                )
...
def update_bed(expanded_scaffold):
    ...
    if not os.path.isfile(
        args.directory + "/alignment_iteration_" + str(iteration + 1) + ".bed"
    ):
        output = open(
            args.directory + "/alignment_iteration_" + str(iteration + 1) + ".bed", "w"
        )
        with open(args.directory + "/alignment_iteration_1.bed", "r") as f:
            for line in f:
                if prev_line == "":
                    ...
                    if (
                        prev_contig in contig2scaffold
                        and curr_contig in contig2scaffold
                    ):
            ...
            output.close()
    len_output = open(
        args.directory + "/scaffold_length_iteration_" + str(iteration + 1), "w"
    )
    ...
    for key in scaffold_re:
        re_out.write(
            key
            + "\t"
            + str(scaffold_re[key][0])
            + "\t"
            + str(scaffold_re[key][1])
            + "\n"
        )
    re_out.close()
...
breakpoints = open(
    args.directory + "/breakpoints_iteration_" + str(iteration + 1) + ".txt", "w"
)
prev_len = {}
...
if iteration > 1:
    for key in previous_scaffolds:
        ...
        if key not in scaffolded_contigs:
            # print(key)
            expanded_scaffold_paths[
                "scaffold_" + str(scaffold_id)
            ] = previous_scaffolds[key]
            scaffold_id += 1
...
if iteration == 1:
    for contig in contig_length:
        if contig not in scaffolded_contigs:
            # print(contig)
            expanded_scaffold_paths["scaffold_" + str(scaffold_id)] = [
                contig + ":B",
                contig + ":E",
            ]
            scaffold_id += 1
...
pickle.dump(
    expanded_scaffold_paths,
    open(args.directory + "/scaffolds_iteration_" + str(args.iteration) + ".p", "w"),
)
